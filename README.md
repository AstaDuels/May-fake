--[[
    ASTA REMAKE – Full Hub (Simplificado) + ESP DO STROM DUELS
    Speed Booster (LinearVelocity) integrado ao Carry Speed e Lagger.
    Botões móveis individuais + fundo personalizado.
    Botão de Lock na barra superior.
    Reset aprimorado (substitui Insta Reset e Medusa Reset).
    Bypass TP + Anti Die.
    Drop com ApplyImpulse.
    ANTI-KICK ROBUSTO (bloqueia qualquer velocidade enquanto pet estiver equipado).
    ANTI-RAGDOLL SIMPLES (forceReset com cooldown).
    BAT V2 integrado (opcional).
    ESP DO STROM DUELS (Box, Highlight, Tracers, Speed, Ragdoll Cooldown)
]]
...
if isfile and isfile("AstaRemake.json") then
    local ok, data = pcall(function() return HS:JSONDecode(readfile("AstaRemake.json")) end)
    if ok and type(data) == "table" and data.introSoundEnabled ~= nil then
        introSoundEnabled = data.introSoundEnabled
    end
end
if isfile and isfile("AstaRemake.json") then
    local ok2, d2 = pcall(function() return HS:JSONDecode(readfile("AstaRemake.json")) end)
...
local CANDY_SKY_PRESETS = {
    ...
    ["Toxic"]={clock=13,brightness=2.5,ambient={180,230,80},outAmb={190,240,90},atm={dens=0.55,color={100,220,40},decay={60,150,20},glare=1.8,haze=2.6},clouds={cover=0.65,dens=0.7,color={180,255,120}}},
...
-- ============================================================
-- SPEED BOOSTER (LinearVelocity) – CORRIGIDO
-- ============================================================
local linearVelocity = nil
local lvAttachment = nil
local savedWalkSpeed = NS
local speedBoosterConn = nil

-- Função auxiliar para obter a velocidade efetiva do booster
local function getEffectiveBoosterSpeed()
    if laggerModeEnabled then
        return LAGGER_CARRY_SPEED
    elseif carrySpeedActive then
        return CS
    else
        return NS  -- fallback (não deve ser usado)
    end
end

local function destroyLV()
    if linearVelocity and linearVelocity.Parent then linearVelocity:Destroy() end
    if lvAttachment and lvAttachment.Parent then lvAttachment:Destroy() end
    linearVelocity = nil; lvAttachment = nil
end

local function setupLV(hrp)
    destroyLV()
    lvAttachment = Instance.new("Attachment")
    lvAttachment.Name = "AstaSpeedAtt"
...
local function applySpeedBoost()
    ...
    if not linearVelocity or linearVelocity.Parent ~= hrp then setupLV(hrp) end
end
...
-- Loop do booster (agora usa a velocidade correta)
local function startSpeedBoosterLoop()
    if speedBoosterConn then speedBoosterConn:Disconnect() end
    speedBoosterConn = RunService.Heartbeat:Connect(function()
        local shouldActivate = carrySpeedActive or laggerModeEnabled
        ...
        local speed = getEffectiveBoosterSpeed()
        if not linearVelocity or linearVelocity.Parent ~= hrp then setupLV(hrp) end

        local moveDir = hum.MoveDirection
        if moveDir.Magnitude > 0.1 then
            local flat = Vector3.new(moveDir.X, 0, moveDir.Z).Unit
            linearVelocity.PlaneVelocity = Vector2.new(flat.X * speed, flat.Z * speed)
        else
            linearVelocity.PlaneVelocity = Vector2.zero
        end
    end)
...
-- Função centralizada para atualizar o booster
local function updateBooster()
    local shouldActivate = carrySpeedActive or laggerModeEnabled
    if shouldActivate then
        applySpeedBoost()
        startSpeedBoosterLoop()
    else
        removeSpeedBoost()
...
-- ============================================================
-- ANTI-RAGDOLL SIMPLES (substitui a versão antiga)
-- ============================================================
...
-- Função auxiliar para ser usada nos toggles
...
LP.CharacterAdded:Connect(function(char)
    task.wait(0.5)
    if antiRagdollEnabled then setAntiRagdoll(true) end
end)

-- ============================================================
-- NOVA FUNÇÃO DROP (com ApplyImpulse)
-- ============================================================
...
local function applyImpulseForVelocity(hrp, desiredVelocity)
    if not hrp then return end
...
local function applyImpulseZero(hrp)
    if not hrp then return end
    local mass = hrp.AssemblyMass
    local currentVel = hrp.AssemblyLinearVelocity
...
function runDrop()
    if not dropEnabled then
        print("Drop está desativado.")
        return
    end
    if dropActive then
        print("Drop já em execução.")
        return
    end
...
-- ============================================================
-- SISTEMA DE RESET (substitui Insta Reset e Medusa Reset) – MAIS RÁPIDO
-- ============================================================
...
local RESET_MAX_DURATION = 0.025  -- reduzido de 0.05 para 0.025
...
-- Keybind para reset (padrão: R)
local resetKeybind = Enum.KeyCode.R
-- Carregar keybind salvo (se houver)
if isfile and isfile("AstaRemake.json") then
    local ok, d = pcall(function() return HS:JSONDecode(readfile("AstaRemake.json")) end)
    if ok and type(d)=="table" and type(d.resetKeybind)=="string" then
        local ok2, kc = pcall(function() return Enum.KeyCode[d.resetKeybind] end)
        if ok2 and kc and kc ~= Enum.KeyCode.Unknown then resetKeybind = kc end
    end
end
...
UIS.InputBegan:Connect(function(inp, gp)
    ...
    if inp.KeyCode == resetKeybind then
        ResetPlayer()
    end
end)

-- ============================================================
-- INFJUMP, MEDUSA, BAT, ETC
-- ============================================================
...
local ASTA={antiKick=false}  -- removido medusaReset
...
-- ============================================================
-- BAT V2 (Aimbot + perseguição automática)
-- ============================================================
local batV2Enabled = false
local batV2Conn = nil
local batV2CD = false
local batV2Rot = nil
local batV2SwingCD = 0.35
local batV2HitDist = 8
...
local function swingBatV2()
    if batV2CD then return end
    batV2CD = true
    pcall(function()
        local char = LP.Character
        if not char then return end
    ...
    task.delay(batV2SwingCD, function() batV2CD = false end)
end

local function updateBatV2()
    ...
    local target, targetDist = getClosestTargetV2()
    if not target then return end
    ...
    if targetDist <= batV2HitDist then swingBatV2() end
end
...
function startBatV2()
    ...
    local hum = LP.Character and LP.Character:FindFirstChildOfClass("Humanoid")
    if hum then
        batV2Rot = hum.AutoRotate
        hum.AutoRotate = false
    end
    batV2Enabled = true
    batV2Conn = RunService.Heartbeat:Connect(updateBatV2)
end
...
function stopBatV2()
    ...
    local hum = char and char:FindFirstChildOfClass("Humanoid")
    if hum then
        hum.AutoRotate = (batV2Rot == nil) and true or batV2Rot
        pcall(function() hum:ChangeState(Enum.HumanoidStateType.GettingUp) end)
    end
    batV2Rot = nil
end
...
local function onHealthChanged(newHealth)
    ...
    if newHealth <= 0 then healPlayer() end
end
...
if LP.Character then
    task.wait(0.2)
    setupAntiDie(LP.Character)
end

function toggleAntiDie()
    antiDieEnabled = not antiDieEnabled
    if antiDieSetVisual then antiDieSetVisual(antiDieEnabled) end
    return antiDieEnabled
end

-- ============================================================
-- CYBER EXTRAS (background + animações)
-- ============================================================
...
-- ============================================================
-- MOBILE BUTTONS – INDIVIDUAIS ARRASTÁVEIS (sem aimbot)
-- ============================================================
local MOB_POS_FILE = "AstaBtnPos.json"
...
local function buildMobileButtons()
    ...
    local savedPos = loadBtnPositions()
    ...
    local buttonOrder = {
        ...
        {key="reset",      label="RESET",       toggle=false},
        {key="tpLock",     label="BYPASS TP",   toggle=true},
        {key="batV2",      label="BAT V2",      toggle=true},
    }

    local function makeButton(def, index)
        local xPos = startX + (index % 3) * (BTN_SIZE + 16)
        local yPos = startY + math.floor(index / 3) * (BTN_SIZE + 16)
        local saved = savedPos[def.key]
        local initX = saved and saved.xo or xPos
        local initY = saved and saved.yo or yPos
        ...
        Instance.new("UICorner", frame).CornerRadius = UDim.new(0, math.max(6, math.floor(BTN_SIZE*0.2)))
        ...
        local function setOn(v)
            isOn = v
            if v then
                TS:Create(frame, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(160,160,160)}):Play()
                TS:Create(stroke, TweenInfo.new(0.12), {Color = Color3.fromRGB(80,80,85), Thickness=2.5}):Play()
                btn.TextColor3 = Color3.fromRGB(0,0,0)
            else
                TS:Create(frame, TweenInfo.new(0.12), {BackgroundColor3 = Color3.fromRGB(10,10,10)}):Play()
                TS:Create(stroke, TweenInfo.new(0.12), {Color = Color3.fromRGB(40,40,45), Thickness=1.5}):Play()
                btn.TextColor3 = Color3.fromRGB(255,255,255)
            end
        end
        ...
        btn.InputChanged:Connect(function(input)
            if dragData.down and not uiLocked and perButtonDragEnabled and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
                ...
                if delta.Magnitude > 10 then dragData.moved = true end
                if dragData.moved then
                    frame.Position = UDim2.new(dragData.startPos.X.Scale, dragData.startPos.X.Offset + delta.X,
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            if def.key == "reset" then ResetPlayer(); return end
        end)
...
-- ============================================================
-- ESP DO STROM DUELS (adicionado na Visual)
-- ============================================================
local ESP_ACCENT = Color3.fromRGB(160, 80, 255)
local ESP_RAGDOLL_CD = 4.5

-- Estado do ESP
local espBoxEnabled = false
local espHighlightEnabled = false
local espTracerEnabled = false
local espSpeedEnabled = false
local espRagdollCDEnabled = false

-- Variáveis internas do ESP
local PlayerESP = { enabled = false, playerData = {}, conns = {} }
local BoxedESPOptions = { box = false, tracer = false }
local BoxedESPData = {}
...
local function isRagdollStateESP(hum)
...
-- RAGDOLL TRACKING
local function espTrackRagdoll(plr)
    if espRagConns[plr] then return end
    espRagConns[plr] = {}
    ...
    local function hook(char)
        ...
        if espRagConns[plr] and espRagConns[plr].state then
            pcall(function() espRagConns[plr].state:Disconnect() end)
        end
        
        local was = isRagdollStateESP(hum)
        espRagConns[plr].state = hum.StateChanged:Connect(function()
            local now = isRagdollStateESP(hum)
    ...
    espRagConns[plr].added = plr.CharacterAdded:Connect(function(c)
        task.defer(hook, plr, c)
    end)
    
    if plr.Character then
        task.defer(hook, plr.Character)
    end
end

local function espUntrackRagdoll(plr)
    local t = espRagConns[plr]
    if t then
...
local function espRagText(plr, hum)
    if not espRagdollCDEnabled then return nil end
    if hum and isRagdollStateESP(hum) then return "ragdoll" end
    local last = espRagLast[plr]
    local left = last and (ESP_RAGDOLL_CD - (tick() - last)) or 0
    if left > 0 then
        return string.format("cd %.1fs", left)
    end
    return "ready"
end

-- PLAYER ESP (HIGHLIGHT + BILLBOARD)
function startPlayerESP()
    ...
    local function setup(plr, char)
        ...
        local conn = RunService.Heartbeat:Connect(function()
            ...
            if d and d.highlight then
                d.highlight.Enabled = espHighlightEnabled == true
            end
            local parts = {}
            if espSpeedEnabled then
                local v = hrp.AssemblyLinearVelocity or hrp.Velocity
                table.insert(parts, string.format("%d speed", math.floor(Vector3.new(v.X, 0, v.Z).Magnitude + 0.5)))
            end
            local rt = espRagText(plr, char:FindFirstChildOfClass("Humanoid"))
            if rt then
                table.insert(parts, rt)
            end
            sub.Text = table.concat(parts, " | ")
            local show = (#parts > 0) or espHighlightEnabled
            n.Visible = show
            sub.Visible = #parts > 0
        end)
    ...
    table.insert(
        ...
        Players.PlayerRemoving:Connect(function(plr)
            if PlayerESP.cleanup then
                PlayerESP.cleanup(plr)
            end
        end)
    )
end
            ...
            if not root or not head then
                ...
                if not data then
                    data = {
                        box = _safeDrawing(
                            "Square",
                            { Thickness = 2, Filled = false, Transparency = 1, Color = ESP_ACCENT }
                        ),
                        tracer = _safeDrawing("Line", { Thickness = 2, Transparency = 1, Color = ESP_ACCENT }),
                    }
...
Players.PlayerRemoving:Connect(function(plr)
    _cleanupBoxedESPPlayer(plr)
    espUntrackRagdoll(plr)
end)

Players.PlayerAdded:Connect(function(plr)
    if espRagdollCDEnabled then
        espTrackRagdoll(plr)
    end
end)

function espApply()
    BoxedESPOptions.box = espBoxEnabled == true
    BoxedESPOptions.tracer = espTracerEnabled == true
    refreshBoxedESP()
    if espRagdollCDEnabled then
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= LP then
        ...
        for plr, _ in pairs(espRagConns) do
            espUntrackRagdoll(plr)
        end
    end
    if espHighlightEnabled or espSpeedEnabled or espRagdollCDEnabled then
        startPlayerESP()
    else
        stopPlayerESP()
    end
...
-- ============================================================
-- FUNÇÕES EXISTENTES (anti-ragdoll, steal, etc)
-- ============================================================
...
local function createRagdollBillboard(duration,labelText,color)
    ...
    local W,H   = 210,80
    local guiName="AstaRagdollTimer_"..labelText
    pcall(function()
...
local function setupSpeedIndicator(char)
    ...
    if head:FindFirstChild("AstaSpeedBB") then head.AstaSpeedBB:Destroy() end
    local bb=Instance.new("BillboardGui",head);bb.Name="AstaSpeedBB";bb.Size=UDim2.new(0,140,0,52);bb.StudsOffset=Vector3.new(0,3,0);bb.AlwaysOnTop=true
    local discordLabel=Instance.new("TextLabel",bb);discordLabel.Size=UDim2.new(1,0,0.4,0);discordLabel.BackgroundTransparency=1;discordLabel.Text="/asta.remake"
...
local KB={AutoLeft={kb=nil,gp=nil},AutoRight={kb=nil,gp=nil},TPFloor={kb=nil,gp=nil},Reset={kb=nil,gp=nil},GuiHide={kb=nil,gp=nil},SpeedToggle={kb=nil,gp=nil},LaggerToggle={kb=nil,gp=nil}}
...
RunService.RenderStepped:Connect(function()
    ...
    if not carrySpeedActive and not laggerModeEnabled then
...
LP.CharacterAdded:Connect(function(char)
    ...
    if carrySpeedActive or laggerModeEnabled then task.wait(0.2);updateBooster() end
    if refreshSpeedModeLabel then refreshSpeedModeLabel() end
    if mobBtnRefs.carrySpeed then mobBtnRefs.carrySpeed(carrySpeedActive) end
    if mobBtnRefs.lagger then mobBtnRefs.lagger(laggerModeEnabled) end
    if mobBtnRefs.tpLock then mobBtnRefs.tpLock(bypassTPEnabled) end
...
local STRETCH_NAME="Asta_Stretch"
...
-- ============================================================
-- ANTI KICK (Asta) – VERSÃO ROBUSTA 100%
-- ============================================================
function ASTA.enableAntiKick()
    ASTA.antiKick=true
    task.spawn(function()
        while ASTA.antiKick do
            task.wait(0.1) -- Verifica mais rápido (0.1s) para reagir instantaneamente
            local char=LP.Character
            if char then
                ...
                brainrotDetected = hasBrainrot

                if hasBrainrot then
                    -- ===== FORÇA DESATIVAR TODA E QUALQUER VELOCIDADE EXTRA =====
                    ...
                    -- Desativa paths automáticos também
                    if autoLeftEnabled then autoLeftEnabled=false;stopAutoLeft();if autoLeftSetVisual then autoLeftSetVisual(false) end;if mobBtnRefs.autoLeft then mobBtnRefs.autoLeft(false) end end
                    if autoRightEnabled then autoRightEnabled=false;stopAutoRight();if autoRightSetVisual then autoRightSetVisual(false) end;if mobBtnRefs.autoRight then mobBtnRefs.autoRight(false) end end
                end
            end
...
-- ============================================================
-- SAVE CONFIG (atualizado com Bat V2 e ESP)
-- ============================================================
saveConfig = function()
    local function ks(e)
    ...
    local cfg = {
        ...
        laggerToggleKey = ks(KB.LaggerToggle), tpFloorKey = ks(KB.TPFloor),
        resetKey = ks(KB.Reset), guiHideKey = ks(KB.GuiHide), speedToggleKey = ks(KB.SpeedToggle),
        ...
        circleButtonsEnabled = circleButtonsEnabled, autoSwitchSpeed = autoSwitchSpeedEnabled,
        fovValue = fovValue, perButtonDrag = perButtonDragEnabled,
        skyTheme = currentSkyTheme, antiKick = ASTA.antiKick,
        autoMoveSwing = autoMoveSwingEnabled, autoMoveSwingInterval = autoMoveSwingInterval,
        ragdollGui = ragdollGuiEnabled, introSoundEnabled = introSoundEnabled,
        animEnabled = animEnabled, backgroundEnabled = backgroundEnabled, backgroundIndex = backgroundIndex,
        keys = (function() if not _GuiKeys then return {} end; local t = {}; for k, v in pairs(_GuiKeys) do t[k] = v.Name end; return t end)(),
        bypassTPEnabled = bypassTPEnabled,
        antiDieEnabled = antiDieEnabled,
        dropEnabled = dropEnabled,
        resetKeybind = resetKeybind.Name,
        batV2Enabled = batV2Enabled,
        -- ESP Configs
        espBox = espBoxEnabled,
        espHighlight = espHighlightEnabled,
        espTracer = espTracerEnabled,
        espSpeed = espSpeedEnabled,
        espRagdollCD = espRagdollCDEnabled,
    }
    if writefile then pcall(function() writefile("AstaRemake.json", HS:JSONEncode(cfg)) end) end
end
...
local function resetAllSettings()
    ...
    ASTA.disableAntiKick()
    autoLeftEnabled=false;autoRightEnabled=false;autoSwingEnabled=true;autoMoveSwingEnabled=false
    ...
    antiDieEnabled=true
    dropEnabled=true
    resetKeybind = Enum.KeyCode.R
    batV2Enabled = false
    KB.AutoLeft={kb=nil,gp=nil};KB.AutoRight={kb=nil,gp=nil}
    KB.TPFloor={kb=nil,gp=nil};KB.Reset={kb=nil,gp=nil}
    ...
    espSpeedEnabled = false
    espRagdollCDEnabled = false
    if espApply then espApply() end
    if carrySpeedActive or laggerModeEnabled then updateBooster() end
    if refreshSpeedModeLabel then refreshSpeedModeLabel() end
    if mobBtnRefs.carrySpeed then mobBtnRefs.carrySpeed(carrySpeedActive) end
    if mobBtnRefs.lagger then mobBtnRefs.lagger(laggerModeEnabled) end
    if mobBtnRefs.autoLeft then mobBtnRefs.autoLeft(false) end
    ...
    if mobBtnRefs.tpLock then mobBtnRefs.tpLock(bypassTPEnabled) end
    if mobBtnRefs.batV2 then mobBtnRefs.batV2(batV2Enabled) end
    stopAutoSteal();stopAutoLeft();stopAutoRight();stopAntiRagdoll();stopAutoTP();stopHoldInfJump()
...
-- ============================================================
-- ANTI-KICK: TOGGLES BLOQUEADOS (robustos)
-- ============================================================
toggleCarryMode=function()
    if brainrotDetected then 
        print("⚠️ Anti-Kick: Carry Speed bloqueado com pet equipado!")
        return 
    end
...
toggleLaggerMode=function()
    if brainrotDetected then 
        print("⚠️ Anti-Kick: Lagger Mode bloqueado com pet equipado!")
        return 
    end
...
local function speedToggleAction() 
    if brainrotDetected then 
        print("⚠️ Anti-Kick: Ação de velocidade bloqueada (tecla Q)!")
        return 
    end
    toggleCarryMode() 
    saveConfig() 
...
local function createStealBar()
    for _,n in ipairs({"AstaStealBar"}) do
    ...
    local stealGui=Instance.new("ScreenGui");stealGui.Name="AstaStealBar";stealGui.ResetOnSpawn=false;stealGui.IgnoreGuiInset=true;stealGui.DisplayOrder=8
...
-- ============================================================
-- FULL CONFIG LOAD (incluindo resetKeybind, dropEnabled, batV2Enabled e ESP)
-- ============================================================
pcall(function()
    if not(isfile and isfile("AstaRemake.json")) then return end
    local ok,d=pcall(function() return HS:JSONDecode(readfile("AstaRemake.json")) end)
    ...
    if type(d.antiKick)=="boolean" then ASTA.antiKick=d.antiKick end
    ...
    if type(d.bypassTPEnabled)=="boolean" then bypassTPEnabled=d.bypassTPEnabled end
    if type(d.antiDieEnabled)=="boolean" then antiDieEnabled=d.antiDieEnabled end
    if type(d.dropEnabled)=="boolean" then dropEnabled=d.dropEnabled end
    if type(d.resetKeybind)=="string" then
        local ok2, kc = pcall(function() return Enum.KeyCode[d.resetKeybind] end)
        if ok2 and kc and kc ~= Enum.KeyCode.Unknown then resetKeybind = kc end
    end
    if type(d.batV2Enabled)=="boolean" then batV2Enabled=d.batV2Enabled end
    -- ESP Configs
    if type(d.espBox)=="boolean" then espBoxEnabled=d.espBox end
    if type(d.espHighlight)=="boolean" then espHighlightEnabled=d.espHighlight end
    if type(d.espTracer)=="boolean" then espTracerEnabled=d.espTracer end
    if type(d.espSpeed)=="boolean" then espSpeedEnabled=d.espSpeed end
    if type(d.espRagdollCD)=="boolean" then espRagdollCDEnabled=d.espRagdollCD end
end)
...
pcall(function()
    ...
    if antiRagdollEnabled then task.spawn(function() task.wait(0.5); setAntiRagdoll(true) end) end
    ...
    if ASTA.antiKick then task.spawn(function() task.wait(1); ASTA.antiKick=false; ASTA.enableAntiKick(); if ASTA.setAntiKickVisual then ASTA.setAntiKickVisual(true) end end) end
    ...
    if carrySpeedActive or laggerModeEnabled then task.spawn(function() task.wait(0.8); updateBooster() end) end
    if bypassTPEnabled then task.spawn(function() task.wait(1); startBypassTP() end) end
    if antiDieEnabled then task.spawn(function() task.wait(0.5); setupAntiDie(LP.Character) end) end
    if batV2Enabled then task.spawn(function() task.wait(0.5); startBatV2() end) end
    -- Aplicar ESP
    task.spawn(function()
        task.wait(1)
        if espApply then espApply() end
...
-- ============================================================
-- GUI PRINCIPAL (ASTA REMAKE) – COM ESP NA VISUAL
-- ============================================================
;(function()
...
;(function()
    ...
    GuiHub.Name="AstaRemakeHub"; GuiHub.ResetOnSpawn=false
    GuiHub.ZIndexBehavior=Enum.ZIndexBehavior.Sibling; GuiHub.Parent=PlayerGui
    GuiRefs.hub=GuiHub

    local Outer=Instance.new("Frame")
    Outer.Name="Outer"; Outer.Size=UDim2.new(0,340,0,520); Outer.Position=UDim2.new(0,6,0,54) -- Aumentado para caber ESP
    ...
    BgImg.Image = "rbxassetid://85719239686756"
    BgImg.ScaleType=Enum.ScaleType.Crop; BgImg.ZIndex=0
    BgImg.ImageTransparency=0.45; BgImg.Visible=true
    ...
    TL.Text="ASTA REMAKE"; TL.TextColor3=C.text; TL.TextSize=17; TL.Font=Enum.Font.GothamBlack
    ...
    ML.Text="ASTA REMAKE • PREMIUM"; ML.TextColor3=C.textDim; ML.TextSize=10; ML.Font=Enum.Font.GothamBold
    ML.TextXAlignment=Enum.TextXAlignment.Left; ML.Parent=HF; ML.ZIndex=3

    -- BOTÃO DE UNLOCK/LOCK
    local lockBtn = Instance.new("TextButton")
    lockBtn.Size = UDim2.new(0, 28, 0, 28)
    lockBtn.Position = UDim2.new(1, -70, 0, 8)
    lockBtn.BackgroundColor3 = uiLocked and Color3.fromRGB(60,200,80) or Color3.fromRGB(50,50,50)
    ...
    lockBtn.Parent = HF
    guiCorner(lockBtn, 7)
    guiStroke(lockBtn, Color3.fromRGB(80,80,80), 1)
    ...
    MiniBtn.Text="ASTA REMAKE"; MiniBtn.TextColor3=C.text; MiniBtn.Font=Enum.Font.GothamBlack; MiniBtn.TextSize=11
...
local function addToggleRow(parent,label,enabled,order,kbKey,onToggle)
    ...
    Track.BorderSizePixel=0; guiCorner(Track,10); guiStroke(Track,Color3.fromRGB(55,55,60),1)
    ...
    Knob.BackgroundColor3=enabled and Color3.fromRGB(80,200,255) or Color3.fromRGB(180,180,180)
    Knob.BackgroundTransparency=enabled and 0 or 0.2
    Knob.BorderSizePixel=0; guiCorner(Knob,7)
    local st=enabled
    local function setV(on)
        st=on
        tw(Knob,{Position=on and UDim2.new(0.5,2,0.5,-7) or UDim2.new(0,2,0.5,-7)})
        tw(Knob,{BackgroundColor3=on and Color3.fromRGB(80,200,255) or Color3.fromRGB(180,180,180)})
        tw(Knob,{BackgroundTransparency=on and 0 or 0.2})
    end
...
;(function()
    ...
    addInputRow(sp,"Carry Speed (Booster)",CS,2,function(v) CS=v; saveConfig() end)
    ...
    addToggleRow(sp,"Carry Mode (Booster)",carrySpeedActive,7,"carryMode",function(on)
        carrySpeedActive = on
        updateBooster()
        if mobBtnRefs.carrySpeed then mobBtnRefs.carrySpeed(carrySpeedActive) end
        if refreshSpeedModeLabel then refreshSpeedModeLabel() end
    ...
    addToggleRow(sp,"Lagger Mode",laggerModeEnabled,8,"laggerToggle",function(on)
        ...
        if on and carrySpeedActive then
            carrySpeedActive=false
            updateBooster()
        elseif not on then
            carrySpeedActive = _prevCarryBeforeLagger or false
            updateBooster()
        end
...
;(function()
    ...
    addSectLbl(cp,"RAGDOLL & COUNTERS",0)

    local _,svRagdoll=addToggleRow(cp,"Anti Ragdoll",antiRagdollEnabled,1,nil,function(on)
    ...
    local _,svMedusa=addToggleRow(cp,"Medusa Counter",medusaCounterEnabled,2,nil,function(on)
    ...
    local _,svUnwalk=addToggleRow(cp,"Unwalk",unwalkEnabled,3,nil,function(on)
        unwalkEnabled=on;if on then startUnwalk() else stopUnwalk() end;saveConfig()
    end)
    setUnwalkVisual=svUnwalk
    addSectLbl(cp,"PROTECTION",4)
    local _,svAntiKick=addToggleRow(cp,"Anti Kick",ASTA.antiKick,5,nil,function(on)
        if on then ASTA.antiKick=false; ASTA.enableAntiKick() else ASTA.disableAntiKick() end
        saveConfig()
    end)
    ASTA.setAntiKickVisual=svAntiKick

    addSectLbl(cp,"ACTIONS",6)
    addActionRow(cp,"Drop","dropBrainrot",function() runDrop() end,7)
    addActionRow(cp,"Reset","reset",function() ResetPlayer() end,8)
    addActionRow(cp,"TP Down","tpDown",function() runTPFloor() end,9)

    addSectLbl(cp,"BAT V2",10)
    local _, svBatV2 = addToggleRow(cp, "Bat V2", batV2Enabled, 11, "batV2", function(on)
        if on then startBatV2() else stopBatV2() end
        if mobBtnRefs.batV2 then mobBtnRefs.batV2(on) end
        saveConfig()
    end)

    local _, svDropEnabled = addToggleRow(cp, "Drop Enabled", dropEnabled, 12, nil, function(on)
        dropEnabled = on
        saveConfig()
    end)
end)()
...
-- VISUAL PAGE – COM ESP ADICIONADO
;(function()
    ...
    antiDieSetVisual = addToggleRow(vi,"Anti Die (auto heal)",antiDieEnabled,6,nil,function(on)
        antiDieEnabled = on
        if on then setupAntiDie(LP.Character) end
        saveConfig()
    end)

    bypassSetVisual = addToggleRow(vi,"Bypass TP",bypassTPEnabled,7,nil,function(on)
        toggleBypassTP()
        if mobBtnRefs.tpLock then mobBtnRefs.tpLock(bypassTPEnabled) end
        saveConfig()
    end)
    ...
    -- ============================================================
    -- SEÇÃO ESP (adicionada do Strom Duels)
    -- ============================================================
    addSectLbl(vi,"ESP (Strom Duels)",7.5)
    
    local espBoxSetVisual = addToggleRow(vi,"Box ESP",espBoxEnabled,8,nil,function(on)
        espBoxEnabled = on
        espApply()
        saveConfig()
    end)
    
    local espHighlightSetVisual = addToggleRow(vi,"Highlight Skin",espHighlightEnabled,9,nil,function(on)
        espHighlightEnabled = on
        espApply()
        saveConfig()
    end)
    
    local espTracerSetVisual = addToggleRow(vi,"Tracers",espTracerEnabled,10,nil,function(on)
        espTracerEnabled = on
        espApply()
        saveConfig()
    end)
    
    local espSpeedSetVisual = addToggleRow(vi,"Speed ESP",espSpeedEnabled,11,nil,function(on)
        espSpeedEnabled = on
        espApply()
        saveConfig()
    end)
    
    local espRagdollSetVisual = addToggleRow(vi,"Ragdoll Cooldown",espRagdollCDEnabled,12,nil,function(on)
        espRagdollCDEnabled = on
        espApply()
        saveConfig()
    end)

    -- ============================================================
    addSectLbl(vi,"SKY THEME",13)
    ...
    skyRow.BackgroundTransparency=0.5; skyRow.BorderSizePixel=0; skyRow.LayoutOrder=14; skyRow.Parent=vi
    ...
    addSectLbl(vi,"FOV",15)
    local fovRow=Instance.new("Frame"); fovRow.Size=UDim2.new(1,0,0,38); fovRow.BackgroundColor3=C.row
    fovRow.BackgroundTransparency=0.5; fovRow.BorderSizePixel=0; fovRow.LayoutOrder=16; fovRow.Parent=vi
    ...
    addSectLbl(vi,"GUI",17)
    local gRow=Instance.new("Frame"); gRow.Size=UDim2.new(1,0,0,42); gRow.BackgroundColor3=C.row
    gRow.BackgroundTransparency=0.5; gRow.BorderSizePixel=0; gRow.LayoutOrder=18; gRow.Parent=vi
    ...
    addToggleRow(vi,"Lock UI (freeze drag)",uiLocked,21,nil,function(on)
        uiLocked = on
        if lockBtn then
            lockBtn.Text = uiLocked and "🔒" or "🔓"
            lockBtn.BackgroundColor3 = uiLocked and Color3.fromRGB(60,200,80) or Color3.fromRGB(50,50,50)
    ...
    addSectLbl(vi,"RESET",22)
    local resetRow=Instance.new("Frame"); resetRow.Size=UDim2.new(1,0,0,38); resetRow.BackgroundColor3=C.row
    resetRow.BackgroundTransparency=0.5; resetRow.BorderSizePixel=0; resetRow.LayoutOrder=23; resetRow.Parent=vi
...
UIS.InputBegan:Connect(function(inp,gp)
    ...
    elseif inp.KeyCode==Keys.reset then ResetPlayer()
    elseif inp.KeyCode==Keys.tpDown then runTPFloor()
    elseif inp.KeyCode==Keys.batV2 then
        local newState = toggleBatV2()
        if svBatV2 then svBatV2(newState) end
        if mobBtnRefs.batV2 then mobBtnRefs.batV2(newState) end
        saveConfig()
...
if ASTA.antiKick then ASTA.antiKick=false; ASTA.enableAntiKick() end
if batV2Enabled then startBatV2() end
CandyApplyCustomSky(currentSkyTheme)
buildMobileButtons()
espApply() -- Aplica o ESP

end)() -- end GUI function

print("✅ ASTA REMAKE LOADED – Anti-Kick 100% robusto + Bat V2 (tecla B) + ESP do Strom Duels na Visual!")
